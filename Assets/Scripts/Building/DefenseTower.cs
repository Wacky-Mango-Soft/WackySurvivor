...
{
    [SerializeField] private string towerName; // 방어 타워의 이름
    ...
    private float currentRateOfFire; // 연사속도 계싼
    ...
    private bool isFindTarget = false; // 적 발견시 true
        ...
        if (isAttack)
        {
            ...
            if (currentRateOfFire >= rateOfFire)
            {
                ...
                if (Physics.Raycast(tf_TopGun.position,
                                    tf_TopGun.forward + new Vector3(UnityEngine.Random.Range(-1f, 1f) * rateOfAccurasy, UnityEngine.Random.Range(-1f, 1f) * rateOfAccurasy, 0f),
